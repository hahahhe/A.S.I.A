from flask import Blueprint, Response, jsonify, request, current_app, url_for
import io
import os
...
from services.audio import audio_file_counts, generate_sound_from_caption, id_mapping
    ...
    try:
        ...
        for class_name in capture_images.keys():
            for capture in capture_images[class_name]:
                image_path = os.path.join(current_app.config['UPLOAD_FOLDER'], capture['capture_filename'])
                ...
                sound_path = os.path.join(current_app.config['UPLOAD_FOLDER'], sound_filename)
                ...
                capture['sound_url'] = url_for('video.uploaded_file', filename=sound_filename)
...
def save_audio():
    ...
    print('app[saved_folder]: ', current_app.config['SAVED_FOLDER'])
    ...
    # 저장된 오디오 파일 목록 반환
    path = os.path.join(current_app.config['SAVED_FOLDER'])
